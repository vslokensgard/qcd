...
# Purpose:    wrapper function to run multiple bins of data through idPlateau()
...
#             None if getPlateauVal() call fails
def plotPlateauFit(ycoords, yerr, min_slice, max_slice, delta=None):
...
def testAltFits(coords, err, static_bound, varied_bound, frac_delta=None):
    if varied_bound <= static_bound:
        ...
        delta_e = oneDimJacknife(coords[varied_bound:static_bound], avg=rho_bar)
        if delta_e <= 0:
            print("Plateau fit from slices",varied_bound,"to",static_bound,"was not calculated due to junk values")
            return -1
        ...
        while varied_bound > 0:
            ...
            if (rho_bar-bound) <= include_next_plateau <= (rho_bar+bound):
                ...
                plotPlateauFit(coords, err, varied_bound, static_bound, delta=bound)
        ...
        while varied_bound < len(coords):
            ...
            if (rho_bar-bound) <= include_next_plateau <= (rho_bar+bound):
                varied_bound = next_slice
                rho_bar = getPlateauVal(coords, err, static_bound, varied_bound)
                plotPlateauFit(coords, err, static_bound, varied_bound, delta=bound)
...
# Purpose:    wrapper function to call testAltFits() on multiple data bins
...
#             optional boolean parameter INIT_PLATEAU, if true display plateau fit from index returned by idPlateau()
...
# Returns:    dictionary storing results of testAltFits() for the selected bins:
...
def listFits(binned, err, selected_bins=None, length=3, max_fact=10, init_plateau = False, vary_min=True, vary_max=False, delta_fact=None):
    ...
    if vary_min:
        adj_min = np.zeros((len(to_plot), 2))
    else:
        adj_min = None
    if vary_max:
        adj_max = np.zeros((len(to_plot), 2))
    else:
        adj_max = None
    if init_plateau:
    ...
    for bin_no in to_plot:
        ...
        plateau_min = idPlateau(bin_data, err, length=length)
        plateau_max = wq.maxSlice(err, max_fact=max_fact)
        
        if init_plateau:
            fit_vals[bin_no] = plotPlateauFit(bin_data, err, plateau_min, plateau_max)
        if vary_min:
            adj_min[bin_no] = testAltFits(bin_data, err, plateau_max, plateau_min, delta_fact)
        if vary_max:
            adj_max[bin_no] = testAltFits(bin_data, err, plateau_min, plateau_max, delta_fact)
      
    return {"plateau":fit_vals, "min":adj_min, "max":adj_max}